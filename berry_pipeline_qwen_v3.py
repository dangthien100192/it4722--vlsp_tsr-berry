...
class Config:
    ...
    output_file: str = "predictions.optimized.json"
    ...
    def from_env(cls) -> "Config":
        cfg = cls(
            ...
            output_file=os.getenv("OUTPUT_FILE", "predictions.optimized.json"),
...
CHECKPOINT_FILE = "example_index_checkpoint.json"
...
def clean_law_text_for_embedding(text: str) -> str:
    text = str(text or "")
...
def get_owl():
    ...
    model = Owlv2ForObjectDetection.from_pretrained(model_dir, local_files_only=True)
    model = model.to(DEVICE)
    model.eval()
...
# =========================================================
# Question intent and object helpers
# =========================================================
...
def score_law_against_choices(item: Dict[str, Any], payload: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
    ...
    debug = {
        ...
        "sign_codes": extract_sign_codes(str(payload.get("title", ""))),
    }
    ...
    for rank, hit in enumerate(retrieved_laws[:5], 1):
        ...
        for label, choice_norm in norm_choice_map.items():
            if choice_norm and choice_norm in combined_norm:
                scores[label] += 0.45 * rank_weight

        for label in matched_choices:
            if label in scores:
                scores[label] += 0.25 * rank_weight

        # nhẹ tay với tín hiệu ảnh: chỉ cộng nhỏ khi detector thật sự nói rõ
        ...
        for label in scores:
            scores[label] += min(fused_score, 1.2) * 0.06 * rank_weight
...
def choose_by_law_priority(item: Dict[str, Any], retrieved_laws: List[Dict[str, Any]]) -> Tuple[Optional[str], Dict[str, Any]]:
    ...
    if exact_title_match and top_gap >= 0.12:
        debug["reason"] = "exact_title_match"
        return best_label, debug

    if best_score >= 0.90 and choice_gap >= 0.25:
        debug["reason"] = "strong_choice_score_gap"
        return best_label, debug

    matched_choices = top1.get("debug", {}).get("matched_choices", []) or []
    if len(matched_choices) == 1 and matched_choices[0] in choice_map and top_gap >= 0.10:
...
# Indexing
# =========================================================
def load_last_success_idx() -> int:
    if not os.path.exists(CHECKPOINT_FILE):
        return -1
    try:
        with open(CHECKPOINT_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        return int(data.get("last_success_idx", -1))
    except Exception as e:
        print(f"[WARN][CHECKPOINT] load failed: {e}")
        return -1


def save_last_success_idx(idx: int) -> None:
    try:
        with open(CHECKPOINT_FILE, "w", encoding="utf-8") as f:
            json.dump({"last_success_idx": idx}, f, ensure_ascii=False, indent=2)
    except Exception as e:
...
def index_examples(client: QdrantClient, dataset: QaDataset) -> None:
    ...
    last_success_idx = load_last_success_idx()
    ...
    print(f"[RESUME][EXAMPLE] last_success_idx={last_success_idx}, start_idx={start_idx}")
    ...
    for idx, item in enumerate(dataset):
        ...
        try:
            ...
            object_labels_en = filter_detected_labels_by_intent(
                detect_objects(image_path, text_queries=get_question_guided_owl_queries(item)),
                item
            )
    ...
    print(f"[DONE][EXAMPLE] skipped={skip_count} | success={success_count} | failed={fail_count}")
...
def index_laws(client: QdrantClient, dataset: LawDataset) -> None:
    ...
    for idx, item in enumerate(dataset):
        ...
        try:
            text = clean_law_text_for_embedding(str(item.get("text", "")).strip()) or "[EMPTY]"
            full_text = clean_law_text_for_embedding(str(item.get("full_text", "")).strip())
            law_title = str(item.get("law_title", "")).strip()
            title = str(item.get("title", "")).strip()

            semantics = build_law_semantics(item)

            # version optimized: embed luật gốc sạch, không nhồi semantics tiếng Anh
            embedding_text = truncate_for_embedding(
                clean_law_text_for_embedding("\n".join([law_title, title, full_text or text])),
                4000,
            )
            text_vec = embed_text_passage(embedding_text)

            payload_text = full_text if full_text else text
...
def retrieve_examples_and_laws(
    ...
    if filtered_labels_vi and config.use_image_models:
        example_weights = (0.60, 0.20, 0.20)
        if question_intent.get("topic") == "parking_restriction":
            example_weights = (0.58, 0.17, 0.25)
...
def build_prompt(
    ...
    def extract_variant_hints(retrieved_laws: List[Dict[str, Any]]) -> List[str]:
        ...
        for hit in retrieved_laws[:3]:
            ...
            if "p.131" in normalize_vi_text(title) or "cấm đỗ xe" in normalize_vi_text(title):
                if "ngày lẻ" in text_norm and "ngày chẵn" in text_norm:
                    hints.append("Trong nhóm P.131, ngày lẻ và ngày chẵn là các biến thể khác nhau.")
        ...
        return unique_keep_order(hints)[:4]
    ...
    lines.append("Ưu tiên điều luật và đặc trưng trực quan then chốt của ảnh.")
    lines.append("Nếu luật có nhiều biến thể a/b/c thì phải phân biệt đúng biến thể.")

    if yes_no:
        lines.append("Chỉ trả lời đúng một từ: ĐÚNG hoặc SAI.")
    else:
        lines.append("Chỉ trả lời đúng một chữ cái: A, B, C hoặc D.")
    ...
    if retrieved_laws:
        ...
        for i, hit in enumerate(retrieved_laws[:5], 1):
            ...
            text = compact_text(clean_law_text_for_embedding(p.get("text", "") or p.get("full_text", "")), 850)
...
def call_llm_openai(prompt: str, question_type: str = "") -> Tuple[str, str]:
    system_prompt = (
        "Bạn là trợ lý giải bài MLQA-TSR về luật giao thông Việt Nam. "
        "Ưu tiên điều luật và các đặc trưng trực quan quan trọng trong ảnh. "
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"[LLM ERROR] status={response.status_code} body={response.text[:1000]}")
...
def compute_rule_confidence(
    ...
    if yes_no:
        ...
        conf += min(support_gap / 0.50, 1.0) * 0.55
        conf += min(max(support_true, support_false) / 0.60, 1.0) * 0.20
        ...
        conf += min(best_score / 1.20, 1.0) * 0.25
        conf += min(max(choice_gap, 0.0) / 0.40, 1.0) * 0.20
        conf += min(max(top_gap, 0.0) / 0.20, 1.0) * 0.10
...
def compute_llm_confidence(
    ...
    if not llm_prediction or not raw:
        return 0.0
    ...
    if retrieved_laws:
        ...
        conf += min(top_score / 1.0, 1.0) * 0.12
        conf += min(max(top_score - second_score, 0.0) / 0.15, 1.0) * 0.05

    if yes_no and any(tok in upper for tok in ["ĐÚNG", "SAI", "TRUE", "FALSE"]):
        conf += 0.05
...
def fuse_predictions(
    ...
    # version optimized: mặc định ưu tiên LLM, chỉ để rule thắng khi cực rõ
    if rule_prediction and llm_prediction and rule_prediction == llm_prediction:
        return rule_prediction, "rule_llm_agree", scores, margin

    if rule_prediction and rule_confidence >= 0.92 and llm_confidence <= 0.55:
        return rule_prediction, "rule_extreme", scores, margin

    if llm_prediction:
        return llm_prediction, "llm_primary", scores, margin

    if rule_prediction:
        return rule_prediction, "rule_fallback", scores, margin

    return best_label if best_score > 0 else fallback, "default_fallback", scores, margin
...
def run_eval(client: QdrantClient, dataset: QaDataset) -> List[Dict[str, Any]]:
    ...
    with open(file_path, "a+", encoding="utf-8") as f:
        ...
        for idx, item in enumerate(dataset, 1):
            ...
            llm_prediction, raw_output = call_llm_openai(prompt, question_type=item.get("question_type", ""))
...
def main() -> None:
    ...
    if config.use_image_models:
        ...
        print("[INFO] Image models disabled -> image/object vectors will be zero or minimal signals.")
    ...
    print(f"[INFO] OpenAI-compatible base URL: {config.openai_base_url}")
    print(f"[INFO] Ollama model fallback: {config.ollama_model}")
