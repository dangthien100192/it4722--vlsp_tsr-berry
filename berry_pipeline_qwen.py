...
def embed_image(image_path: str) -> List[float]:
    ...
    try:
        model = CRADIO_MODEL if CRADIO_MODEL is not None else get_c_radio()
        pixel_values = preprocess_c_radio_image(image)
        ...
        summary = summary[0].detach().float().cpu().numpy()
        return ensure_dim(l2_normalize_np(summary), IMAGE_DIM)

    except Exception as e:
        print(f"[WARN] embed_image failed for {image_path}: {e}")
        return [0.0] * IMAGE_DIM
...
def build_prompt(
    ...
    lines.append(
        ...
        "Ưu tiên bám sát điều luật và ví dụ truy hồi. "
    ...
    if retrieved_laws:
        ...
        for i, hit in enumerate(retrieved_laws, 1):
            ...
            lines.append(f"[LAW {i}] {law_id} | {article_id} | {title}\n{snippet}")
