...
class Config:
    ...
    def from_env(cls) -> "Config":
        cfg = cls(
            ...
            llm_num_predict=int(os.getenv("LLM_NUM_PREDICT", "24")),
...
def index_examples(client: QdrantClient, dataset: QaDataset) -> None:
    ...
    for idx, item in enumerate(dataset):
        ...
        try:
            ...
            print(f"[text_vec][EXAMPLE] {idx}: done")

            image_vec = embed_image(image_path)
            print(f"[image_vec][EXAMPLE] {idx}: done")

            object_labels_en = filter_detected_labels_by_intent(
                detect_objects(
                    image_path,
                    text_queries=get_question_guided_owl_queries(item)
                ),
                item
            )
            ...
            print(f"[object_vec][EXAMPLE] {idx}: done")
    ...
    print(
        f"[DONE][EXAMPLE] skipped={skip_count} | success={success_count} | failed={fail_count}"
    )
...
def build_law_embedding_text(item: Dict[str, Any], semantics: Dict[str, Any]) -> str:
    ...
    law_title = str(item.get("law_title", "") or "")
    title = str(item.get("title", "") or "")
    text = clean_law_text_for_embedding(str(item.get("text", "") or ""))
    full_text = clean_law_text_for_embedding(str(item.get("full_text", "") or ""))
...
def build_prompt(
    ...
    if retrieved_laws:
        lines.append("\n# ĐIỀU LUẬT THAM KHẢO")
        for i, hit in enumerate(retrieved_laws[:3], 1):
            p = hit.get("payload", {})
            title = str(p.get("title", "")).strip()
            law_id = str(p.get("law_id", "")).strip()
            article_id = str(p.get("article_id", "")).strip()
            text = compact_text(clean_law_text_for_embedding(p.get("text", "") or p.get("full_text", "")), 1200)
